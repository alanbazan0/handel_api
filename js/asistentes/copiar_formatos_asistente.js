class CopiarFormatosAsistente
{
	constructor()
	{
		var date = new Date();
		this._modal = "copiarFormatosModal" + date.getTime();
		this._contexto = null;
		this._funcion = null;
...
//		this._iconosModal = new IconosModal();
	...
	renderizarModal()
	{
		...
		html+="				<h5 class='modal-title' id='"+this._modal +"Titulo'>Copiar formatos</h5>";
	...
	onShowStep(obj,context)
	{
		...
		else if(context.toStep==3)
		{
			...
			if(procedimientosSeleccionados.length==0)
			{
				swal({
    	            title: "Error",
    	            text: "No hay formatos seleccionados, seleccione al menos uno para continuar",
    	            type: "warning",
    	            confirmButtonColor: "#DD6B55",
    	            confirmButtonText: "Cerrar",
    	            closeOnConfirm: true
	...
	set procesos(procedimientos)
	{
		...
		else
		{
			swal({
	            title: "Error",
	            text: "No exiten formatos en esta sede, seleccione una sede diferente",
	            type: "warning",
	            confirmButtonColor: "#DD6B55",
	            confirmButtonText: "Cerrar",
	            closeOnConfirm: true
...
//		}
	...
	agregarStepsPredeterminados(){
		...
		html+="		<span class='step_descr'>Formatos<br />";
		html+="	    <small>Seleccione los formatos que desea copiar</small>";
		html+="		</span>";
...
//	}
	...
	copiarProcedimientosResultado(resultado)
	{
		...
		{
			$("#"+this._modal).modal('hide');
			this._contexto.mostrarMensaje("Notificación","Los formatos se copiaron correctamente.");
		}
		else
			this._contexto.mostrarMensajeError("Error",resultado.mensajeError);
	}
