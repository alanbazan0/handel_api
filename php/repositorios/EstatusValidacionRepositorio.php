...
use php\interfaces\IEstatusValidacionRepositorio;
use php\modelos\EstatusValidacion;
use php\modelos\Resultado;

require_once("../interfaces/IEstatusValidacionRepositorio.php");
require_once( "../modelos/EstatusValidacion.php");
require_once( "../clases/TipoUsuario.php");
require_once( "RepositorioBase.php");
require_once("../clases/Resultado.php");

class EstatusValidacionRepositorio extends RepositorioBase implements IEstatusValidacionRepositorio
{
    protected $conexion;
    ...
    public function __construct($conexion)
    {
        ...
        $this->consultaBase = " SELECT E.id, E.descripcion " .
            " FROM estatus_validacion E ";
    }
    
    public function insertar(EstatusValidacion $modelo)
    {
        $resultado =  $this->calcularId("id","estatus_validacion");
        if($resultado->mensajeError=="")
        {
            $id = $resultado->valor;
            $consulta = "INSERT INTO estatus_validacion(id, descripcion) " .
                "VALUE(?, ?)";
    ...
    public function actualizar(EstatusValidacion $modelo)
    {
        ...
        $consulta = " UPDATE estatus_validacion " .
            "SET descripcion = ? " .
            "WHERE id = ? ";
...
//         }
    ...
    {
        $resultado = new Resultado();
        $consulta = " DELETE FROM estatus_Validacion "
            . "  WHERE id  = ? ";
            if($sentencia = $this->conexion->prepare($consulta))
            {
                if($sentencia->bind_param("i",$llaves->id))
