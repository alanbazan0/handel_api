...
use php\interfaces\IEstandaresRepositorio;
use php\modelos\Estandar;
use php\modelos\Resultado;

include "../interfaces/IEstandaresRepositorio.php";
include "../modelos/Estandar.php";
include "RepositorioBase.php";
require_once("../clases/Resultado.php");

class EstandaresRepositorio extends RepositorioBase implements IEstandaresRepositorio
{
    protected $conexion;
    ...
    {
        $resultado =  $this->calcularId("id","estandares");
        if($resultado->mensajeError=="")
        {
            $id = $resultado->valor;
            $consulta = "INSERT INTO estandares(id, nombre, fecha_alta, fecha_modificacion, estatus) " .
                "VALUE(?, ?, NOW(), NOW(), ?)";
    ...
    {
        $resultado = new Resultado();
        $consulta = " DELETE FROM estandares "
            . "  WHERE id  = ? ";
            if($sentencia = $this->conexion->prepare($consulta))
            {
                if($sentencia->bind_param("i",$llaves->id))
