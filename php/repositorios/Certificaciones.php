...
use php\repositorios\CertificacionesRepositorio;
...
include '../repositorios/CertificacionesRepositorio.php';
...
try
{
    ...
    if($conexion)
    {
        ...
        $repositorio = new CertificacionesRepositorio($conexion);
        switch ($accion)
        {           
            case 'insertar':               
                $json = json_decode(REQUEST('modelo'));
                $mapper = new JsonMapper();
                $modelo = $mapper->map($json, new Certificacion());                   
