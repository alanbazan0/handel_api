...
class EvidenciasRepositorio extends RepositorioBase implements IEvidenciasRepositorio
{
    ...
    public function __construct($conexion)
    {
        ...
        $this->consultaBase = "SELECT E.id, usuario_procedimiento_id, realizo_actividad, justificacion_id, comentarios, IFNULL(DATE_FORMAT(E.fecha_alta,'%d/%m/%Y %H:%i:%s'),'') as fecha, P.nombre, nombre_archivo, P.codigo, J.nombre,
                                (SELECT count(C.id) FROM evidencias_comentarios C WHERE C.evidencia_id = E.id) numeroComentarios, U.nombre, U.apellido, S.id, S.nombre, EM.id, EM.nombre, U.id, validada, comentarios_validacion, EM.administrador_id, V.nombre administradorNombre, V.apellido administradorApellido,E.validacion_usuario_id validadorId, VL.nombre validadorNombre, VL.apellido validadorApellido, UP.procedimiento_id   
                               FROM evidencias E
...
//         $and = $this->and($filtros);
    ...
    public function consultarPorcentajesCertificaciones($usuario,$criteriosSeleccion)
    {
        ...
        $filtros = $this->getFiltrosN($usuario, $criteriosSeleccion,false);
        $and = $this->and($filtros);
        $consulta = "SELECT certificacionId, certificacionNombre, SUM(justificadas)justificadas, SUM(enviadas)enviadas, SUM(pendientes)pendientes ".
            "\nFROM(" .
            $this->getConsultaEvidenciasBaseCertificaciones($usuario,$criteriosSeleccion,$and)  .
            "\n) AS A " .
            ...
            if($sentencia = $this->conexion->prepare($consulta))
            {
                if($this->bind_param($sentencia, $filtros))
                {
                    if($sentencia->execute())
                    {
                        if($sentencia->bind_result($certificacionId, $certificacionNombre, $justificadas, $enviadas, $pendientes))
                        {
                            while($sentencia->fetch())
                            {
                                ...
                                $registro= (object) [
                                    ...
                                    'nombreId' =>  $certificacionNombre ." (".$certificacionId.")",
                                    'justificadas' =>  $justificadas,
                                    'enviadas' =>  $enviadas,
                                    'pendientes' =>  $pendientes
                                ];
...
//                     	AND UP.id NOT IN(SELECT usuario_procedimiento_id FROM evidencias E WHERE MONTH(E.fecha_alta) = $criteriosSeleccion->mes AND YEAR(E.fecha_alta) = $criteriosSeleccion->ano) ";
    ...
    public function getConsultaEvidenciasBaseCertificaciones($usuario,$criteriosSeleccion,$and,$camposGroupBy=null)
    {
