...
use php\interfaces\IServiciosRepositorio;
use php\modelos\Servicio;
use php\modelos\Resultado;

include '../interfaces/IServiciosRepositorio.php';
include '../modelos/Servicio.php';
include 'RepositorioBase.php';
require_once('../clases/Resultado.php');

class ServiciosRepositorio extends RepositorioBase implements IServiciosRepositorio
{
    protected $conexion;
    ...
    public function __construct($conexion)
    {
        ...
        $this->consultaBase = "SELECT id, RTRIM(nombre) as nombre, IFNULL(DATE_FORMAT(fecha_alta,'%d/%m/%Y %H:%i:%s'),'') as fecha_alta, IFNULL(DATE_FORMAT(fecha_modificacion,'%d/%m/%Y %H:%i:%s'),'') as fecha_modificacion, estatus FROM servicios S";
    }

    public function insertar(Servicio $modelo)
    {
        $resultado = $this->calcularId('id','servicios');
        if($resultado->mensajeError=='')
        {
            $id = $resultado->valor;
            $consulta = "INSERT INTO servicios(id, nombre, fecha_alta, fecha_modificacion, estatus)VALUES(?, ?, NOW(), NOW(), ?)";
    ...
    {
        $resultado = new Resultado();
        $consulta = "DELETE FROM servicios WHERE id = ?";
        if($sentencia = $this->conexion->prepare($consulta))
        {
            if($sentencia->bind_param('i',$llaves->id))
            {
