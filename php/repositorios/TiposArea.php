...
use php\repositorios\TiposAreaRepositorio;
...
include '../repositorios/TiposAreaRepositorio.php';
...
try
{
    ...
    if($conexion)
    {
        ...
        $repositorio = new TiposAreaRepositorio($conexion);
        switch ($accion)
        {           
            case 'insertar':               
                $json = json_decode(REQUEST('modelo'));
                $mapper = new JsonMapper();
                $modelo = $mapper->map($json, new TipoArea());                   
