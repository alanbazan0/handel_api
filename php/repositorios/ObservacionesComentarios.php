...
use php\repositorios\ObservacionesComentariosRepositorio;
...
include '../repositorios/ObservacionesComentariosRepositorio.php';
...
try
{
    ...
    if($usuario!=null)
    {
        ...
        if($conexion)
        {
            ...
            $repositorio = new ObservacionesComentariosRepositorio($conexion);
            switch($accion)
            {
                case 'insertar':
                    $json = json_decode(REQUEST('modelo'));
                    $mapper = new JsonMapper();
                    $modelo = $mapper->map($json, new ObservacionComentario());
