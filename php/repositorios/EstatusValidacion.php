...
use php\repositorios\EstatusValidacionRepositorio;
...
include '../repositorios/EstatusValidacionRepositorio.php';
...
try
{
    ...
    if($conexion)
    {
        ...
        $repositorio = new EstatusValidacionRepositorio($conexion);
        switch ($accion)
        {           
            case 'insertar':               
                $json = json_decode(REQUEST('modelo'));
                $mapper = new JsonMapper();
                $modelo = $mapper->map($json, new EstatusValidacion());                   
