...
use php\repositorios\EstatusValidacionProcesosRepositorio;
...
include '../repositorios/EstatusValidacionProcesosRepositorio.php';
...
try
{
    ...
    if($conexion)
    {
        ...
        $repositorio = new EstatusValidacionProcesosRepositorio($conexion);
        switch ($accion)
        {           
            case 'insertar':               
                $json = json_decode(REQUEST('modelo'));
                $mapper = new JsonMapper();
                $modelo = $mapper->map($json, new EstatusValidacionProceso());                   
