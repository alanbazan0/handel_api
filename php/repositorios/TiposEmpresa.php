...
use php\repositorios\TiposEmpresaRepositorio;
...
include '../repositorios/TiposEmpresaRepositorio.php';
...
try
{
    ...
    if($conexion)
    {
        ...
        $repositorio = new TiposEmpresaRepositorio($conexion);
        switch ($accion)
        {           
            case 'insertar':               
                $json = json_decode(REQUEST('modelo'));
                $mapper = new JsonMapper();
                $modelo = $mapper->map($json, new TipoEmpresa());                   
